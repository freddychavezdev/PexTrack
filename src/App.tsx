...
function Dashboard({ profile, onLogout }: { profile: Profile; onLogout: () => void }) {
  const [dark, setDark] = useState(() => localStorage.theme === 'dark'); const [crews, setCrews] = useState<Crew[]>([]); const [orders, setOrders] = useState<WorkOrder[]>([]); const [selectedOrder, setSelectedOrder] = useState<WorkOrder>(); const [selectedCrew, setSelectedCrew] = useState<Crew>(); const [highlightedOrderId, setHighlightedOrderId] = useState<string>(); const [tab, setTab] = useState<'map' | 'table'>('map'); const [panelOpen, setPanelOpen] = useState(true); const [running, setRunning] = useState(false); const [message, setMessage] = useState(''); const highlightTimer = useRef<number>()
